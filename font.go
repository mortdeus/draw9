...
func (f Font) String() string {
	return fmt.Sprintf("Font %q height %d ascent %d", f.Name, f.Height, f.Ascent)
}
...
func (s Subfont) String() string {
	return fmt.Sprintf("Subfont %s N %d height %d ascent %d ninfo %d ref %d",
		s.Name, s.N, s.Height, s.Ascent, len(s.Info), s.Ref)
}
...
func fontresize(f *Font, wid, ncache, depth int) int {
	...
	new, err = d.allocImage(image.Rect(0, 0, ncache*wid, f.Height), color9.MakePix(color9.CGrey, depth), false, 0)
	if err != nil {
